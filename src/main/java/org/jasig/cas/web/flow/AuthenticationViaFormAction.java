...
import org.wo.cas.core.authentication.BadAuthcodeAuthenticationException;
import org.wo.cas.core.authentication.NullAuthcodeAuthenticationException;
import org.wo.cas.core.authentication.UsernamePasswordCredentialWithAuthCode;
...
public class AuthenticationViaFormAction {
     ...
     * @since 4.1.0
     */
    public final Event submit(final RequestContext context, final Credential credential, final MessageContext messageContext) {
    ...
    protected boolean isRequestAskingForServiceTicket(final RequestContext context) {
        ...
        return (StringUtils.isNotBlank(context.getRequestParameters().get(CasProtocolConstants.PARAMETER_RENEW)) && ticketGrantingTicketId != null && service != null);
    }
    ...
    /**
     * Create ticket granting ticket for the given credentials. Adds all warnings into the message context.
     *
     * @param context
     *            the context
     * @param credential
     ...
     * @since 4.1.0
     */
    protected Event createTicketGrantingTicket(final RequestContext context, final Credential credential, final MessageContext messageContext) {
        try {
            final Service service = WebUtils.getService(context);
            final AuthenticationContextBuilder builder = new DefaultAuthenticationContextBuilder(this.authenticationSystemSupport.getPrincipalElectionStrategy());
            final AuthenticationTransaction transaction = AuthenticationTransaction.wrap(credential);
    ...
    private static void addWarningToContext(final MessageContext context, final MessageDescriptor warning) {
        final MessageBuilder builder = new MessageBuilder().warning().code(warning.getCode()).defaultText(warning.getDefaultMessage()).args(warning.getParams());
